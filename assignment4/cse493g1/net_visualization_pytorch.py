...
def make_fooling_image(X, target_y, model):
    ...
    for epoch in range(iter):
      ...
      pred_score, pred = torch.max(score, axis=1)
      pred_score, pred = pred_score.item(), pred.item()
      target_score = score[:, target_y].squeeze()
      print('iter: %2d ' % (epoch+1) + \
            '- Pred class: %3d (Score: %.2f) ' % (pred, pred_score) + \
            '- Target class: %d (Score: %.2f)' % (target_y, target_score.item()))
      if torch.argmax(score) == target_y:
        print("Succesfully Fooled")
        break
...
def class_visualization_update_step(img, model, target_y, l2_reg, learning_rate):
    ...
    score = model(img)
    target_score = score[:, target_y] - l2_reg * torch.square(torch.norm(img))
