...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        temp, dW_proj, db_proj = affine_backward(dh0, h0_cache)
    ...
    def sample(self, features, max_length=30):
        ...
        h, temp = affine_forward(features, W_proj, b_proj)
        ...
        for ts in range(max_length):
          x, temp = word_embedding_forward(fwords, W_embed)

          if self.cell_type == 'rnn':
            h, temp = rnn_step_forward(x, h, Wx, Wh, b)
          
          hts = np.expand_dims(h, axis=1)

          hidden_temp, temp = temporal_affine_forward(hts, W_vocab, b_vocab)
          hidden_temp = hidden_temp.squeeze()

          fwords = np.argmax(hidden_temp, axis=1)
