...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    hidden_input = prev_h @ Wh + x @ Wx + b
    next_h = np.tanh(hidden_input)
    cache = (x, prev_h, Wx, Wh, b, hidden_input)
...
def rnn_step_backward(dnext_h, cache):
    ...
    x, prev_h, Wx, Wh, b, hidden_input = cache

    dHidden_input = (1 - np.tanh(hidden_input)**2) * dnext_h

    db = dHidden_input.sum(axis = 0)

    dWh = prev_h.T @ dHidden_input
    dprev_h = dHidden_input @ Wh.T

    dWx = x.T @ dHidden_input
    dx = dHidden_input @ Wx.T
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
      x_t = x[:,t,:]
      h_t, t_cache = rnn_step_forward(x_t, prev_h, Wx, Wh, b)
...
def rnn_backward(dh, cache):
    ...
    for t in range(T-1, -1, -1):
      backward_t = rnn_step_backward(dprev_t, cache[t])

      dx[:,t,:] = backward_t[0]
      dprev_h = backward_t[1]
      dWx += backward_t[2]
      dWh += backward_t[3]
      db += backward_t[4]
