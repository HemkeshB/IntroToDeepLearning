...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        attn_weights = torch.softmax(scores, dim=-1)  # Shape: (N, H, S, T)
        attn_weights = self.attn_drop(attn_weights)   # Apply dropout

        # Apply attention weights to values
        attn_output = torch.matmul(attn_weights, V)  # Shape: (N, H, S, E/H)

        # Combine heads
        attn_output = attn_output.transpose(1, 2).contiguous()  # Shape: (N, S, H, E/H)
        attn_output = attn_output.view(N, S, E)                 # Shape: (N, S, E)
