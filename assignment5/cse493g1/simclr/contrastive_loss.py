...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        for i in range(2*N):
            z_i = out[i]
            if i != k:
                sum_left += torch.exp(sim(z_k, z_i) / tau)
            if i != k + N:
                sum_right += torch.exp(sim(z_k_N, z_i) / tau)
        loss_left = -torch.log(torch.exp(sim(z_k, z_k_N) / tau) / sum_left )
        loss_right = -torch.log(torch.exp(sim(z_k_N, z_k) / tau) / sum_right )
...
def sim_positive_pairs(out_left, out_right):
    ...
    pos_pairs = torch.diag(torch.matmul(left_norm, right_norm.transpose(0,1))).reshape(-1, 1)
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    pos_pairs = pos_pairs.repeat(2, 1).to(device)
