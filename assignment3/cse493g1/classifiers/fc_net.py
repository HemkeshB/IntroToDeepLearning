...
class FullyConnectedNetImproved(object):
    ...
    def __init__(
        ...
        for i in range(self.num_layers):
            self.params[f"W{i+1}"] = np.random.randn(layers[i], layers[i+1]) * weight_scale
            self.params[f"b{i+1}"] = np.zeros(layers[i+1])

        if self.normalization != None:
          for i in range(self.num_layers-1):
            self.params[f"gamma{i+1}"] = np.ones(layers[i+1])
            self.params[f"beta{i+1}"] = np.zeros(layers[i+1])
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers-1):  
            ...
            output, cache = affine_batchnorm_relu_forward(output, self.params[f'W{i+1}'],
                                self.params[f'b{i+1}'], gamma, beta, bn_params,
                                self.normalization, self.use_dropout, self.dropout_param)
            caches.append(cache)

        scores, cache = affine_forward(output, self.params[f'W{self.num_layers}'], self.params[f'b{self.num_layers}'])
        caches.append(cache)
        ...
        for i in range(self.num_layers):
          if i != 0:
            dout, dw, db, dgamma, dbeta = affine_batchnorm_relu_backward(dout,
               caches[self.num_layers - i - 1], self.normalization, self.use_dropout)
            if self.normalization != None:
